...
import okhttp3.Call;
/**
 * 文件名：AddProblemTypePopwindow
 * 描    述：添加问题类型的pop
 * 作    者：stt
 * 时    间：2017.3.10
 * 版    本：V1.0.6
 */
public class AddProblemTypePopwindow extends PopupWindow implements View.OnClickListener {
    ...
    public AddProblemTypePopwindow(final Activity context, ArrayList<ProblemTypeLeft> problemTypeLeftLists) {
        super(context);
        this.context = context;
        this.problemTypeLeftList = problemTypeLeftLists;
        View view = View.inflate(context, R.layout.pop_problem_detail_type, null);
...
//		this.setAnimationStyle(R.style.AnimBottom);
    ...
    private class AddressTextAdapter extends AbstractWheelTextAdapter1 {
        ArrayList<ProblemTypeLeft> listLeft;
        ArrayList<ProblemTypeRight> listRight;
        int type = -1;

        protected AddressTextAdapter(Context context, ArrayList<ProblemTypeLeft> list, int currentItem, int maxsize, int minsize) {
            super(context, R.layout.item_wheelview, NO_RESOURCE, currentItem, maxsize, minsize);
            this.listLeft = list;
            setItemTextResource(R.id.item_wheelView);
        }

        protected AddressTextAdapter(Context context, int type, ArrayList<ProblemTypeRight> list, int currentItem, int maxsize, int minsize) {
            super(context, R.layout.item_wheelview, NO_RESOURCE, currentItem, maxsize, minsize);
            this.listRight = list;
            this.type = type;
            setItemTextResource(R.id.item_wheelView);
